...
import com.arka.api.dto.response.ErrorResponse;
...
public class GlobalExceptionHandler {
    ...
    // Triggered by @Validated + @Min/@Positive/@Pattern on @RequestParam / @PathVariable
    ...
    public Mono<ResponseEntity<ErrorResponse>> handleConstraintViolation(ConstraintViolationException ex) {
        String message = ex.getConstraintViolations().stream()
                .map(cv -> {
                    ...
                    // Strip method prefix: "listOrders.status" -> "status"
    ...
    public Mono<ResponseEntity<ErrorResponse>> handleDataIntegrity(DataIntegrityViolationException ex) {
        ...
        return Mono.just(ResponseEntity
                .status(HttpStatus.CONFLICT)
                .body(new ErrorResponse("ORDER_CONSTRAINT_VIOLATION",
                        "Order constraint violated: operation would result in an invalid order state")));
    }
