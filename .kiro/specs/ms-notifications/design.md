...
1. **Puramente event-driven**: Sin endpoints REST. El único entry-point es el consumidor Kafka suscrito a `order-events`, `inventory-events` (Fase 1) y `payment-events` (Fase 2).
...
graph TB
    ...
    subgraph "Strategy (domain/usecase)"
        ...
        ES5[PaymentProcessedStrategy]
        ES6[PaymentFailedStrategy]
    end
...
#### Estrategias de Procesamiento (Fase 1 + Fase 2)

| Estrategia                   | eventType            | Campos extraídos del payload                                                      | Destinatario                        | Fase |
| ---------------------------- | -------------------- | --------------------------------------------------------------------------------- | ----------------------------------- | ---- |
| `OrderConfirmedStrategy`     | `OrderConfirmed`     | orderId, customerId, customerEmail, items (sku, quantity, unitPrice), totalAmount | customerEmail del payload           | 1    |
| `OrderStatusChangedStrategy` | `OrderStatusChanged` | orderId, previousStatus, newStatus, customerEmail                                 | customerEmail del payload           | 1    |
| `OrderCancelledStrategy`     | `OrderCancelled`     | orderId, customerId, customerEmail, reason                                        | customerEmail del payload           | 1    |
| `StockDepletedStrategy`      | `StockDepleted`      | sku, productName, currentQuantity, threshold                                      | Email admin (propiedad Spring Boot) | 1    |
| `PaymentProcessedStrategy`   | `PaymentProcessed`   | orderId, customerId, customerEmail, amount, paymentMethod, transactionId          | customerEmail del payload           | 2    |
| `PaymentFailedStrategy`      | `PaymentFailed`      | orderId, customerId, customerEmail, amount, paymentMethod, reason                 | customerEmail del payload           | 2    |
...
```java
// Consumidor Kafka suscrito a order-events, inventory-events y payment-events
...
// eventTypes relevantes Fase 2: PaymentProcessed, PaymentFailed
// eventTypes desconocidos: log WARN, ignorar (tolerancia a evolución)
```

| Consumer                    | Tópicos                                              | Eventos Procesados                                                                                | Tecnología                            |
| --------------------------- | ---------------------------------------------------- | ------------------------------------------------------------------------------------------------- | ------------------------------------- |
| `KafkaNotificationConsumer` | `order-events`, `inventory-events`, `payment-events` | OrderConfirmed, OrderStatusChanged, OrderCancelled, StockDepleted, PaymentProcessed, PaymentFailed | `KafkaReceiver` (reactor-kafka §B.12) |
...
**Plantillas iniciales (Fase 1 + Fase 2):**

| eventType            | subject                                           | Variables en bodyTemplate                                          | Fase |
| -------------------- | ------------------------------------------------- | ------------------------------------------------------------------ | ---- |
| `OrderConfirmed`     | `Pedido {{orderId}} - Confirmación`               | orderId, customerId, items (sku, quantity, unitPrice), totalAmount | 1    |
| `OrderStatusChanged` | `Pedido {{orderId}} - Estado: {{newStatus}}`      | orderId, previousStatus, newStatus                                 | 1    |
| `OrderCancelled`     | `Pedido {{orderId}} - Cancelado`                  | orderId, reason                                                    | 1    |
| `StockDepleted`      | `Alerta Stock Bajo - SKU {{sku}}`                 | sku, productName, currentQuantity, threshold                       | 1    |
| `PaymentProcessed`   | `Pedido {{orderId}} - Pago Procesado Exitosamente` | orderId, amount, paymentMethod, transactionId                      | 2    |
| `PaymentFailed`      | `Pedido {{orderId}} - Fallo en el Pago`           | orderId, amount, paymentMethod, reason                             | 2    |
...
_Para cualquier_ evento de dominio cuyo `eventType` no pertenezca al conjunto relevante (Fase 1: `OrderConfirmed`, `OrderStatusChanged`, `OrderCancelled`, `StockDepleted`; Fase 2: `PaymentProcessed`, `PaymentFailed`), el sistema debe ignorar el evento sin enviar correo, sin crear registro en `notification_history`, y sin lanzar excepción.
...
_Para cualquier_ evento de tipo OrderConfirmed, OrderStatusChanged, OrderCancelled, PaymentProcessed o PaymentFailed, el correo debe enviarse al `customerEmail` extraído del payload. _Para cualquier_ evento de tipo StockDepleted, el correo debe enviarse a la dirección de email del Administrador configurada en las propiedades de Spring Boot.
...
| ---------------------------------------- | --------------------------------------------------------------------- | ---------------------------------------------------- |
| P1: Idempotencia                         | Generar eventos, procesarlos dos veces, verificar sin duplicados      | eventId, eventType aleatorios del conjunto relevante (6 tipos) |
| P2: Filtrado de eventos desconocidos     | Generar eventTypes fuera del conjunto relevante                       | Strings aleatorios excluyendo los 6 tipos relevantes          |
...
| P4: Estrategia extrae campos correctos   | Generar payloads válidos por tipo de evento                           | Payloads con campos requeridos por eventType (6 tipos)        |
| P5: Email resuelto contiene variables    | Generar eventos con plantilla y payload, verificar resultado          | Plantillas + payloads completos (6 tipos)                     |
| P6: Email al destinatario correcto       | Generar eventos de todos los tipos, verificar destinatario            | Emails aleatorios, eventTypes (6 tipos)                       |
