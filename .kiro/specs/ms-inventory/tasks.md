...
- [x] 16. Corrección de bugs en consumo de eventos Kafka: OrderConfirmed y OrderCancelled
  ...
  - [x] 16.1 Añadir factory method `orderConfirm()` al record `StockMovement` en `domain/model`
  ...
  - [x] 16.2 Añadir `findAllByOrderIdAndStatus()` a `StockReservationRepository` y su implementación R2DBC
  ...
  - [x] 16.3 Implementar `processOrderConfirmed(UUID eventId, UUID orderId)` en `StockReservationUseCase`
  ...
  - [x] 16.4 Corregir `processOrderCancelled()` en `StockReservationUseCase` — eliminar dependencia de `sku`
  ...
  - [x] 16.5 Actualizar `KafkaEventConsumer.handleOrderEvent()` — añadir `OrderConfirmed`, corregir `OrderCancelled`
  ...
  - [x] 16.6 Actualizar `docs/03-kafka-eventos.md` — tabla Consumer Groups MVP Fase 1
